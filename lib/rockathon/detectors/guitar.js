...
Guitar.prototype.warmUpCalcCache = function() {
  this.bxAx = (this.pickHandsMeanX - this.neckHandsMeanX);
  this.byAy = (this.pickHandsMeanY - this.neckHandsMeanY);
};
...
Guitar.prototype.process = function(img) {
  if (this.configuiring > 0) {
    ...
    if (pickHand) {
      ...
      var aboveOrBelow = (this.pickHandsMeanX - this.neckHandsMeanX) * (pickHand[1] - neckHand[1]) - (this.pickHandsMeanY - this.neckHandsMeanY) * (pickHand[0] - neckHand[0]);
      ...
      if (this.pickHandAbove != above) {
        ...
        this.inst.play(notes[note], 10 - octave);
      }
    }
  }
  return img;
